...
def choose():
    if request.method == 'POST':
        ...
        if error is not None:
            ...
            db = get_db()
            try:
            	stock_id = db.execute('SELECT stock_id FROM stock WHERE stock_name = ?', (stock_name,)).fetchone()
...
def get_post():
    try:
        ...
        post = get_db().execute('SELECT stock_id from user_stock WHERE username = ?', (session.get('username'),)).fetchall()
        print(post)
    except Exception as e:
        print(e)
    return post
